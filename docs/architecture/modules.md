...
| **sections** | `components/sections/` | One component per landing-page section (Hero, Reveal, FeatureShowcase, ThemeGallery, Screenshots, WaitlistSignup, Faq, Footer). Each renders content from `lib/content`. Server Component by default; splits into a `*.client.tsx` island only where a section needs scroll/animation state (e.g. `Hero.tsx` → `HeroCanvasMount.client.tsx`/`HeroCanvas.client.tsx`/`HeroLogoReveal.client.tsx`; `Reveal.tsx` → `RevealScene.client.tsx`). `WaitlistSignup.tsx` (S11, TASK-0004) is itself a top-level `"use client"` component (not split into an island) since the entire section is interactive — it owns the `idle\|submitting\|success\|error` form state and calls `lib/waitlist.ts`. `Faq.tsx` (S10, TASK-0004) stays a Server Component — native `<details>`/`<summary>` needs no client JS. Reads `lib/motion.ts` directly (not just via `providers`) to decide reduced-motion/low-power fallbacks per section. |
...
- **WebGL stays capped at two scenes (R-1).** `Hero`'s starfield (TASK-0003) and the
  future S2 hero-character continuation (TASK-0005) are the only two React Three
